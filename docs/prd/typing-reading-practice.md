...
The operator also needs to turn authorized English webpages into deployable Reading Sources. That workflow must preserve source content exactly, use AI only for annotations, require explicit review, and keep AI credentials and runtime services out of the deployed site.
...
- Keep source content and annotations structurally separate. Validate that AI output cannot replace or mutate source text.
